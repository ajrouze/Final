...
double forkProp        = 0.254; double forkInteg       = 0.230;
double rotkProp        = 1.463; double rotkInteg       = 1.328;
double forwardOuterGain = 1;  double rotatOuterGain = 1;
double posK[6] = {forkProp, forkInteg, rotkProp, rotkInteg, forwardOuterGain, rotatOuterGain};
...
static double rhoErrorInteg     = 0;                   // aggregate error of robot linear velocity
static double phiErrorInteg     = 0;                   // aggregate error of robot rotational velocity
...
void posController(double desiredPos, double desiredPhi, double posK[6]);
void resetValues();
void trigCalcs();
...
void loop() {
  ...
  //posController(0.0, 3.14*2.363, posK);
  ...
  switch(state){
    case 0:
      // sweep
      // rotational control
      //    rotate increments of PI/4
      //    if velocities == 0
      //      check if I2C has been called
      //        if true: stop moving, do trig calcs, resetValues(), state = 1
      //        else: reset state to 0, resetValues()
      posController(0.0, (PI/4.0)*angleCorrect, posK);
      if((L_angVel == 0) || (R_angVel == 0)){
        if(i2cFlag){
          resetValues();
          trigCalcs();    //posController(desiredXPos, phi_des*angleCorrect, posK);
          state = 1;
        } else {
          state = 0;
          resetValues();
    ...
    case 2:
      // drive robot forward
      // if velocities == 0
      //    if at final aruco: stop everything (default)
      //    else: resetValues(), state = 0
      break;
    default:
      // write 0 to posController
      resetValues();
      posController(0.0, 0.0, posK);
      break;
  }
...
void velController(double leftDesiredVel, double rightDesiredVel, double leftAngVelNow, double rightAngVelNow, double velK[4]){
  ...
  /* check if right motor voltage is saturated, if so set motor voltage to battery voltage */
  if(rightMotorVoltage > (double)batteryVoltage)
    rightMotorVoltage = (double)batteryVoltage;
  else if(rightMotorVoltage < -(double)batteryVoltage) 
    rightMotorVoltage = -(double)batteryVoltage;  
...
void posController(double desiredPos, double desiredPhi, double posK[6]){
  //C_rho implementation
  double posError = desiredPos - x_now;
  double rhoDotDes = posError * posK[4];
  double rhoDotError = rhoDotDes - J_linVel;

  rhoErrorInteg += rhoDotError * ((double)samplingTime/(double)milli);
  double barV = (rhoDotError * posK[0]) + (rhoErrorInteg * posK[1]);

  //C_phi implementation
  double phiError = desiredPhi - phi_now;
  double phiDotDes = phiError * posK[5];
  double phiDotError = phiDotDes - J_rotVel;

  phiErrorInteg += phiDotError * ((double)samplingTime/(double)milli);
  double deltaV = (phiDotError * posK[2]) + (phiErrorInteg * posK[3]);

  double rightV = (barV + deltaV)/2.0;
  double leftV  = (barV - deltaV)/2.0;

  /* check if right motor voltage is saturated, if so set motor voltage to battery voltage */
  if(rightV > (double)batteryVoltage)
    rightV = (double)batteryVoltage;
  else if(rightV < -(double)batteryVoltage) 
    rightV = -(double)batteryVoltage;  

  /* check if left motor voltage is saturated, if so set motor voltage to battery voltage */
  if(leftV > (double)batteryVoltage)
    leftV = (double)batteryVoltage;
  else if(leftV < -(double)batteryVoltage) 
    leftV = -(double)batteryVoltage; 

  /* convert motor voltage to PWM */
  double rightDutyCycle = ((abs(rightV) / batteryVoltage) * (double)255);
  double leftDutyCycle = ((abs(leftV) / batteryVoltage) * (double)255);
...
void trigCalcs(){
  ...
  double perpDistAruco = cvData[1];
  desiredXPos = square(distancefromAruco) + square(perpDistAruco);

  double cvAngle;
  if (cvData[3] == 0) {
    cvAngle = (double)cvData[2]/(double)100;
  } else {
    cvAngle = -1.0*(double)cvData[2]/(double)100;
  }
  
  double extraAngle = atan2(distancefromAruco, perpDistAruco);
  phi_des = (double)extraAngle + (double)cvAngle;
...
void printData() {
  ...
  //Serial.print("\t"); Serial.print(i2cFlag);
